...
import { HookHQDefaultError } from "../models/errors/hookhqdefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | HookHQDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | HookHQDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new HookHQDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new HookHQDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
