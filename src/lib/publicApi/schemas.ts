...
export const destinationTypeSchema = z.enum(["webhook", "sqs", "pubsub"]).describe('Destination type for the endpoint. See destination configuration schemas for corresponding configuration fields.');
...
}).describe('Webhook destination configuration').openapi("Webhook Destination");
...
}).describe('Amazon SQS destination configuration').openapi("Amazon SQS Destination");

const pubsubDestinationConfigSchema = z.object({
...
}).describe('Google Pub/Sub destination configuration').openapi('Google PubSub Destination');
...
export const endpointUpdateSchema = z
  .object({
    ...
    destination: z
      .union([webhookDestinationConfigSchema, sqsDestinationConfigSchema, pubsubDestinationConfigSchema])
      .optional(),
    enabled: z.boolean().optional().describe('Whether the endpoint is enabled.'),
    retry: retryConfigSchema.partial().optional().describe('Retry configuration for the endpoint.'),
    autoDisable: autoDisableConfigSchema.partial().optional().describe('Auto-disable configuration for the endpoint.'),
...
export const endpointGroupCreateSchema = z
  .object({
    ...
    failureAlerts: failureAlertConfigSchema.partial().optional().describe('Failure alert configuration for the endpoint group.'),
  })
